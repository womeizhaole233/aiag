...
const muralChoiceLayer = document.querySelector("#muralChoiceLayer");
const muralChoiceConfirm = document.querySelector("#muralChoiceConfirm");
const muralChoiceCancel = document.querySelector("#muralChoiceCancel");
const matchGameLayer = document.querySelector("#matchGameLayer");
const matchGameSuccess = document.querySelector("#matchGameSuccess");
let pendingMuralHotspot = null;
...
function load() {
  ...
  try {
    ...
    state.cleanedNorthWestMural = Boolean(data.cleanedNorthWestMural);
    save();
  } catch {
...
const NORTH_WEST_MURAL_HOTSPOT_ID = "north_west_mural";

function isNorthWestMuralHotspot(hotspot) {
  return hotspot?.id === NORTH_WEST_MURAL_HOTSPOT_ID;
}

function openMuralChoice(hotspot) {
  pendingMuralHotspot = hotspot;
  muralChoiceLayer.classList.remove("hidden");
}

function closeMuralChoice() {
  muralChoiceLayer.classList.add("hidden");
  pendingMuralHotspot = null;
}

function confirmMuralChoice() {
  if (!pendingMuralHotspot) return;
  closeMuralChoice();
  openMatchGame();
}

function cancelMuralChoice() {
  closeMuralChoice();
}

function openMuralMessageAfterGame() {
  if (!pendingMuralHotspot) return;
  const hotspot = pendingMuralHotspot;
  pendingMuralHotspot = null;
  openMessage(hotspot);
}

function openMatchGame() {
  matchGameLayer.classList.remove("hidden");
  startMatchGame();
}

function closeMatchGame() {
  matchGameLayer.classList.add("hidden");
  stopMatchGame();
}

function onMatchGameWin() {
  state.cleanedNorthWestMural = true;
  save();
  stopMatchGame();
  matchGameSuccess.classList.remove("hidden");
  setTimeout(() => {
    matchGameSuccess.classList.add("hidden");
    matchGameLayer.classList.add("hidden");
    openMuralMessageAfterGame();
  }, 2000);
}

function onMatchGameExit() {
  closeMatchGame();
  pendingMuralHotspot = null;
}
...
(function initMatchGame() {
  ...
  const CELL_W = 90;
  const CELL_H = 90;
  const ELIMINATE_DELAY = 500;
  const FALL_DURATION = 500;
  const ANIM_PHASE = { IDLE: 0, HIGHLIGHT: 1, FALLING: 2 };
  const TYPE_SOIL = 0;
  const TYPE_STONE = 1;
  const TYPE_SHARD = 2;
  const TYPE_DUST = 3;
  ...
  const DIG_SOUND = "picture/铲子挖土_01_爱给网_aigei_com.mp3";
  ...
  function loadImages(callback) {
    imagesLoaded = 0;
    imagesReady = false;
    for (const type in BLOCK_IMAGES) {
      if (loadedImages[type] && loadedImages[type].complete) {
        imagesLoaded++;
        continue;
      }
      const img = new Image();
  ...
  function updateSoilUI() {
    if (!soilFillDiv || !soilPercentSpan) return;
    const percent = Math.max(0, Math.min(100, 100 - totalEliminated));
    soilFillDiv.style.height = percent + "%";
    soilPercentSpan.textContent = Math.floor(percent) + "%";
  }
  ...
  function stabilizeGridWithoutProgress(grid) {
    ...
    while (anyMatch && loopGuard++ < maxLoop) {
      ...
      for (let c = 0; c < COLS; c++) {
        ...
        const missing = ROWS - columnVals.length;
  ...
  function prepareFallAnimation(matches) {
    const blocks = [];
    const affectedCols = [...new Set(matches.map((m) => m.col))];

    for (const c of affectedCols) {
      const eliminatedRows = matches.filter((m) => m.col === c).map((m) => m.row);
      eliminatedRows.sort((a, b) => a - b);
      const eliminatedCount = eliminatedRows.length;
      const eliminatedSet = new Set(eliminatedRows);

      for (let r = 0; r < ROWS; r++) {
        if (gameGrid[r][c] >= 0 && !eliminatedSet.has(r)) {
          let dropDistance = 0;
          for (const er of eliminatedRows) {
            if (er > r) dropDistance++;
          }
          if (dropDistance > 0) {
            blocks.push({
              col: c,
      ...
      for (let i = 0; i < eliminatedCount; i++) {
        const startRow = -(eliminatedCount - i);
        blocks.push({
          col: c,
          type: getRandomBlockType(),
          startRow: startRow,
          targetRow: startRow + eliminatedCount,
          startY: startRow * CELL_H,
          targetY: (startRow + eliminatedCount) * CELL_H,
    ...
    return new Promise((resolve) => {
      ...
      function step(timestamp) {
        ...
        for (const block of fallingBlocks) {
          const totalDistance = block.targetY - block.startY;
  ...
  function playDigSound() {
    try {
      const sound = new Audio(DIG_SOUND);
      sound.volume = 0.6;
      sound.play().catch(() => {});
    } catch (e) {}
  }
  ...
  async function resolveMatchesAndDrop(updateProgress = true) {
    ...
    while (loopSafe++ < maxLoop) {
      const matches = getAllMatches(gameGrid);
      if (matches.length === 0) break;
  ...
  function drawBlockAt(x, y, type, isHighlight = false) {
    ...
    const blockX = x + gap;
    const blockY = y + gap;
    const blockW = CELL_W - gap * 2;
    const blockH = CELL_H - gap * 2;
    const borderColor = BLOCK_BORDER_COLORS[type] || "#8b6914";
    const borderWidth = 4;
    ...
    ctx.fillRect(blockX + blockW - borderWidth, blockY + borderWidth, borderWidth, blockH - borderWidth * 2);
    ctx.fillRect(blockX + borderWidth, blockY + blockH - borderWidth, blockW - borderWidth * 2, borderWidth);

    const img = loadedImages[type];
    if (img) {
      const imgX = blockX + borderWidth;
      const imgY = blockY + borderWidth;
      const imgW = blockW - borderWidth * 2;
      const imgH = blockH - borderWidth * 2;
  ...
  function drawCanvas() {
    ...
    for (let i = 0; i < ROWS; i++) {
      for (let j = 0; j < COLS; j++) {
        const x = j * CELL_W;
        const y = i * CELL_H;
        const type = gameGrid[i][j];
        const isFallingStart = currentAnimPhase === ANIM_PHASE.FALLING && fallingStartSet.has(`${i},${j}`);
        ...
        if (isFallingStart) {
          ...
          if (type >= 0) {
            const isHighlight = eliminatingBlocks.some((b) => b.row === i && b.col === j);
    ...
    if (currentAnimPhase !== ANIM_PHASE.FALLING && selectedRow >= 0 && selectedCol >= 0) {
      const x = selectedCol * CELL_W;
      const y = selectedRow * CELL_H;
      const selectedType = gameGrid[selectedRow][selectedCol];
      const selectedBorderColor = selectedType >= 0 ? BLOCK_BORDER_COLORS[selectedType] || "#f7d44a" : "#f7d44a";
      ctx.strokeStyle = "#ffffff";
      ctx.lineWidth = 3;
      ctx.shadowBlur = 12;
  ...
  async function handleCanvasClick(e) {
    ...
    const sRow = selectedRow;
    const sCol = selectedCol;
    ...
    isProcessing = true;
    const success = await trySwapAndMatch(sRow, sCol, row, col);
    selectedRow = -1;
    selectedCol = -1;
    drawCanvas();
...
function bindHotspotActivation(element, hotspot) {
  const activate = () => {
    if (isNorthWestMuralHotspot(hotspot) && !state.cleanedNorthWestMural) {
      openMuralChoice(hotspot);
      return;
    }
    openMessage(hotspot);
  };
...
muralChoiceConfirm.addEventListener("click", confirmMuralChoice);
muralChoiceCancel.addEventListener("click", cancelMuralChoice);
muralChoiceLayer.addEventListener("click", (event) => {
  if (event.target === muralChoiceLayer) cancelMuralChoice();
});
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape") {
    ...
    if (!muralChoiceLayer.classList.contains("hidden")) cancelMuralChoice();
    if (!matchGameLayer.classList.contains("hidden")) onMatchGameExit();
